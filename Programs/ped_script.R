...
maf_function<-function(df){                                                       #function to calculate MAF by country
  ...
  qvar<-as.data.frame(table(c(df$Allele_1,df$Allele_2)))
  ...
  for (i in unique(df$Country)){
    alleles<-as.data.frame(table(c(df[df$Country==i,"Allele_1"],
    df[df$Country==i,"Allele_2"])))
    if (MA %in% alleles$Var1){
      ma<-alleles[alleles$Var1==MA,"Freq"]
      maf<-round(ma/sum(alleles$Freq),digits=2)
    }
    else {maf<-0} 
    df[df$Country==i,"MAF_bycountry"]<-maf
  }
  return(df)
}
